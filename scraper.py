...
def nz_scrape_details(url: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        traits = {}
        # National Zoo has "Physical Description", "Food/Eating Habits", etc. as H2s, H3s or H4s
        ...
        # Description: Text before the first H2/H3/H4 that is not the title
        # This is hard to pinpoint without exact classes.
        # But we can try to find the text "Among the largest..." which is likely in a <p> tag.
        if not description:
             # Find the first paragraph that is NOT in the traits
             for p in soup.find_all('p'):
                 text = p.get_text(strip=True)
                 if len(text) > 50 and "Kingdom:" not in text and not any(s in text for s in skip_phrases):
                     # Check if this text is already in traits
                     is_trait = False
